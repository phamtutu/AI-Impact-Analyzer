...
def build_llm():
    return ChatOllama(
        ...
        # Breakdown task cần kết quả ổn định, ít sáng tạo
        temperature=0.1,

        # Tắt thinking để tránh reasoning text làm hỏng JSON structured output
        reasoning=False,

        # Context window. Tăng nếu project-knowledge lớn.
        num_ctx=16384,

        # Số token tối đa model được sinh ra
        num_predict=4096,

        # Timeout cho local model
        client_kwargs={
            "timeout": 300,
        },

        # Kiểm tra model có tồn tại ngay khi khởi tạo
...
"""
        structured_llm = llm.with_structured_output(BreakdownResult,method="json_schema",)
